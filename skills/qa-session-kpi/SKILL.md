...
{
  "messages": [
    {"timestamp": "2026-05-01T10:30:00", "content": "...", "session": "abc123", "project": "atlas-qa-reporter"}
  ],
  "count": 42
}
```
...
**Confluence 게시 옵션:**
사용자가 "Confluence에 올려줘"라고 요청하면, 기존 `atlas-qa-reporter`의 `ConfluenceClient`를 활용하여 `CONFLUENCE_QA_REPORT_PARENT_ID` 하위에 게시한다.
...
**Confluence 게시 실패 시:**
`atlas-qa-reporter`의 `.env` 파일에 `CONFLUENCE_QA_REPORT_PARENT_ID`가 설정되어 있는지 확인한다. 미설정 시 Markdown 파일로만 저장된다. Confluence 인증 실패 시 `python bin/debug_jira.py`로 연결 상태를 확인한다.
...
이 스킬은 `atlas-qa-reporter`의 기존 클라이언트를 재사용할 수 있다:
...
| 프로젝트 필터 | 전체 프로젝트 | "atlas-qa-reporter만 봐줘" |
